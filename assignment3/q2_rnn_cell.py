...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x', shape=(self.input_size, self.state_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
            W_h = tf.get_variable('W_h', shape=(self.state_size, self.state_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable('b', shape=self.state_size,dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
            x_t = inputs
            h_t_1 = state
            new_state = tf.nn.sigmoid(tf.matmul(x_t,W_x)+tf.matmul(h_t_1,W_h)+b)
